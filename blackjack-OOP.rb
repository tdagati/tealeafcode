...
module Hand
	...
	def total
		face_values = cards.map{|card| cards.value }
		...
		#correct for Aces
		faces_values.select{|val| val == "A"}.count.times deal_one
			break if total <= 21
			total -= 10
		end
...
class Blackjack
	...
	def blackjack_or_bust?(player_or_dealer)
		if player_or_dealer.total == BLACKJACK_AMAOUNT
			if player-or_dealer.is_a?(Dealer)
				puts "Sorry, dealer hit blakjack. #{player.name} loses."
			else
				puts "Congratulations, you hit blackjack! #{player.name} wins!"
			end
			play_again?
		elsif player_or_dealer.is_busted?
			if player_or_dealer_is.a?(Dealer)
